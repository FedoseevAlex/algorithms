...
class Solution:
    def maxCandies(
        ...
        opened = set()
        closed = {box for box in initialBoxes if status[box] == 0}
        queue = [box for box in initialBoxes if status[box] == 1]
        while queue:
            box = queue.pop()
            ...
            opened.add(box)
            candies_num += candies[box]
            got_keys.update(keys[box])
            ...
            for new_box in containedBoxes[box]:
                if status[new_box] == 0 and new_box not in got_keys:
                    closed.add(new_box)
                else:
...
if __name__ == "__main__":
    ...
    s = Solution()
    for idx, (status, candies, keys, containedBoxes, initialBoxes, expected) in enumerate(test_cases):
