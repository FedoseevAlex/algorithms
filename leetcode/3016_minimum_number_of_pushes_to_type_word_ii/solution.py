...
def solution(word: str) -> int:
    ...
    for idx, count in enumerate(counter):
        result += (1 + idx // 8) * count

    return result

# beats only 19% for runtime
def _solution(word: str) -> int:
    counter = {}
    buttons = 8
    for letter in word:
        count = counter.get(letter, 0)
        counter[letter] = count + 1
    sorted_letters = {letter: idx for idx, letter in enumerate(sorted(counter.keys(), reverse=True, key=lambda x: counter[x]))}
    return sum(count * (1 + sorted_letters[letter] // buttons) for letter, count in counter.items())
